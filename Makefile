...
build:
	docker buildx build --no-cache --platform linux/amd64,linux/arm64 -t $(DOCKER_ACCOUNT)/$(IMAGE_NAME):$(IMAGE_VERSION) --push .

build-amd64:
	docker buildx build --no-cache --platform linux/amd64 -t $(DOCKER_ACCOUNT)/$(IMAGE_NAME)-amd64:$(IMAGE_VERSION) --push .

build-arm64:
	docker buildx build --no-cache --platform linux/arm64 -t $(DOCKER_ACCOUNT)/$(IMAGE_NAME)-amd64:$(IMAGE_VERSION)  --push .
